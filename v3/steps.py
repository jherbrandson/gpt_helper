...
def step1(config, suppress_output=False):
    ...
    if config.get("has_single_root"):
        ...
        for d in directories:
            ...
            if is_remote:
                ...
                lines = [directory_root] + custom_tree(directory_root, prefix="", level=1, max_level=999)
    ...
    extra_content = ""
    if config.get("has_single_root"):
        # Single-root: read from config["project_output_files"]
        ...
        # Multi-root: each directory has its own 'output_files' list
...
def step2_all_segments(config, suppress_output=False):
    ...
    for i, seg in enumerate(segments):
        ...
        # Determine if this segment is remote.
        is_remote = seg.get("is_remote", False)
        ssh_cmd = config.get("ssh_command", "") if is_remote else ""
        # For remote segments, pass the full blacklist dictionary and project_root.
        if is_remote:
        ...
        if selected_files:
            # For local systems, reapply the blacklist filter to the final file list.
