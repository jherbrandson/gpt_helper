...
def run_content_setup(config):
    ...
    def proceed_file_selection():
        if config.get("has_single_root"):
            ...
            config["project_output_files"] = get_selected_files(tree)
        else:
            for idx, tree in selection_trees.items():
                config["directories"][idx]["output_files"] = get_selected_files(tree)
    ...
    # Updated build_local_tree_widget with double-click binding for toggling selection.
